...
```python
numeric = pd.to_numeric(df['col'], errors='coerce')  # unparseable -> NaN
invalid_numeric = df['col'].notna() & numeric.isna()   # identify failed conversions
dates = pd.to_datetime(df['date'], errors='coerce')    # unparseable -> NaT
```
