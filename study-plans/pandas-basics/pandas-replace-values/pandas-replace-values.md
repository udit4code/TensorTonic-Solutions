...
## <span style="font-size: 16px;">Forward and Backward Fill</span>

<span style="font-size: 14px;">Use the dedicated fill methods when missing values should be propagated from neighboring rows:</span>
...
<span style="font-size: 14px;">Use `replace()` for explicit value-to-value substitutions. Use `ffill()` or `bfill()` when missing values should be propagated from surrounding data.</span>
