...
class LSTMCell(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        # Input Gate : Controls how much of the candidate information should be written into the cell state.
        ...
        # Cell Candidate : Produces new candidate information that may be written into the cell state.
        ...
        # Output Gate : Controls how much of the cell state becomes the hidden state exposed to the next layer/time step.
    ...
    def forward(self, x, h_prev, c_prev):
        """
            ...
            Returns:
                (h_t, c_t)
        """
        # Input Gate : Determines how much new information should enter the cell.
        i_t = torch.sigmoid(
            x @ self.W_ii.T +
            self.b_ii +
            h_prev @ self.W_hi.T +
        ...
        # Forget Gate :  Determines how much previous memory should be retained.
        f_t = torch.sigmoid(
            x @ self.W_if.T +
            self.b_if +
            h_prev @ self.W_hf.T +
        ...
        # Cell Candidate : Computes new candidate memory values.
        g_t = torch.tanh(
            x @ self.W_ig.T +
            self.b_ig +
            h_prev @ self.W_hg.T +
        ...
        # Output Gate : Determines how much of the cell state should be exposed as the hidden state.
        o_t = torch.sigmoid(
            x @ self.W_io.T +
            self.b_io +
            h_prev @ self.W_ho.T +
