...
def streaming_minmax_update(state, X_batch, eps=1e-8):
    ...
    """
    # Safely convert input to a NumPy array if a list is passed
    X_batch = np.asarray(X_batch, dtype=np.float64)
    
    if X_batch.size == 0:
        return X_batch

    # Calculate batch boundaries along the instance tracking dimension
    batch_min = np.min(X_batch, axis=0)
    batch_max = np.max(X_batch, axis=0)

    # Perform running state adjustments
    state["min"] = np.minimum(state["min"], batch_min)
    state["max"] = np.maximum(state["max"], batch_max)

    # Scale the current incoming block using updated running limits
